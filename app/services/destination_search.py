...
async def _fetch_destinations(
    *,
    ...
    radius_km: int,
    keyword_type: DestinationKeywordType,
    count: int,
    client: TMapClient,
    ...
    params = {
        ...
        "searchType": "all",
        "searchtypCd": (
            "R" if keyword_type is DestinationKeywordType.CATEGORY else "A"
        ),
        "centerLat": str(departure_latitude),
        "centerLon": str(departure_longitude),
    ...
    data = await client.get(path="/pois", params=params)
...
def _deduplicate_destinations(
    ...
    unique_destinations: list[DestinationCandidate] = []
...
async def search_destinations(
    ...
    *,
    ...
    if radius_expand_factor <= 1.0:
        raise ValueError("radius_expand_factor는 1보다 커야 합니다.")

    tmap_client = client or TMapClient()
    destinations = await _fetch_destinations(
        keyword=normalized_keyword,
        departure_latitude=departure_latitude,
        departure_longitude=departure_longitude,
        radius_km=initial_radius_km,
        keyword_type=keyword_type,
        count=poi_fetch_count,
        client=tmap_client,
    )
    destinations = _deduplicate_destinations(destinations)
    ...
    if len(destinations) < candidate_count:
        ...
        if expanded_radius_km > initial_radius_km:
            expanded = await _fetch_destinations(
                keyword=normalized_keyword,
                departure_latitude=departure_latitude,
                departure_longitude=departure_longitude,
            ...
            destinations = _deduplicate_destinations([*destinations, *expanded])

    if keyword_type is DestinationKeywordType.CATEGORY:
        destinations.sort(
